...
contract ProofChain is Initializable {
    ...
    bytes32 public constant BLOCK_SPECIMEN_PRODUCER_ROLE =
        keccak256("BLOCK_SPECIMEN_PRODUCER_ROLE");
    bytes32 public constant STAKING_ORACLE_ROLE =
        keccak256("STAKING_ORACLE_ROLE");
    ...
    CountersUpgradeable.Counter private proofSeq;
    ...
    function getRequiredStakeForRole(bytes32 role)
        public
        view
        returns (uint256)
    {
    ...
    function setRequiredStakeForRole(bytes32 role, uint256 amount) public {
        ...
        if (prevRequiredStakeForRole < amount) {
            ...
            uint256 numStakers = stakers.length();
            for (uint256 i = 0; i < numStakers; i++) {
                grantRoleIfPreapprovedAndSufficientlyStaked(
                    role,
                    stakers.at(i)
                );
            }
    ...
    function setStakedBalance(address addr, uint256 amount) public {
        ...
        if (prevStakedBalance < amount) {
            ...
            uint256 numRolesWithRequiredStake = rolesWithRequiredStake.length();
            for (uint256 i = 0; i < numRolesWithRequiredStake; i++) {
                revokeRoleIfNoLongerSufficientlyStaked(
                    rolesWithRequiredStake.at(i),
                    addr
                );
            }
    ...
    function isPreapprovedForRole(bytes32 role, address account)
        public
        view
        returns (bool)
    {
    ...
    function isSufficientlyStakedForRole(bytes32 role)
        public
        view
        returns (bool)
    {
    ...
    function isSufficientlyStakedForRole(bytes32 role, address account)
        public
        view
        returns (bool)
    {
    ...
    function grantRoleIfPreapprovedAndSufficientlyStaked(
        bytes32 role,
        address account
    ) private {
        if (
            isPreapprovedForRole(role, account) &&
            isSufficientlyStakedForRole(role, account)
        ) {
            roles[role].members[account] = true;
            emit RoleGranted(role, account, msg.sender);
        }
    ...
    function revokeRoleIfNoLongerSufficientlyStaked(
        bytes32 role,
        address account
    ) private {
