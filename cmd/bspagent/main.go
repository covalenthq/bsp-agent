...
func consumeEvents(config *config.Config, avroCodecs *goavro.Codec, redisClient *redis.Client, pinClient *pinner.Client, storageClient *storage.Client, ethClient *ethclient.Client, consumerName, streamKey, consumerGroup string) {
	for {
		...
		for _, stream := range streams[0].Messages {
			waitGrp.Add(1)
			go processStream(config, avroCodecs, redisClient, pinClient, storageClient, ethClient, stream, streamKey, consumerGroup)
		}
		waitGrp.Wait()
	}
}
...
func processStream(config *config.Config, replicaCodec *goavro.Codec, redisClient *redis.Client, pinClient *pinner.Client, storageClient *storage.Client, ethClient *ethclient.Client, stream redis.XMessage, streamKey, consumerGroup string) {
